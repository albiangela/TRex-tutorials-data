...
def tile_with_yolo_tiler(
    ...
    dst: str | Path,
    *,
    input_ext: str = ".jpg",          # you can keep this arg for your own logic if you want
    ...
    # decide whether we want negative (empty) tiles
    need_negatives = (negative_fraction or 0.0) > 0.0 if negative_fraction is not None else include_empty

    cfg = TileConfig(
        slice_wh=slice_wh,
        overlap_wh=overlap_wh,
        # input_ext is no longer a valid TileConfig argument in new yolo_tiler
        # images are auto-detected; if you want to force output format, use output_ext=".jpg"
        output_ext=None,
...
def prepare_yolo_dataset(
    ...
    *,
    ...
    def rebalance_dataset(
        ...
        """Re-split dataset into train/valid/test.

        Supports two layouts for `dataset_root`:

        A) Classic split root:
           dataset_root/
        ...
        In both cases, all image/label pairs are collected and then
        re-split into train/valid/test in `output_path`.
        """
        ...
        # ---- Try layout A: dataset_root/{train,valid,test}/images,labels ----
        subset_dirs: List[Tuple[str, str, str]] = []
        for subset in ("train", "valid", "test"):
            ...
            if os.path.isdir(img_dir) and os.path.isdir(lbl_dir):
                subset_dirs.append((subset, img_dir, lbl_dir))

        if subset_dirs:
            # Layout A: gather from each subset
            for subset, img_dir, lbl_dir in subset_dirs:
            ...
            # ---- Try layout B: dataset_root/images, dataset_root/labels ----
            img_dir = os.path.join(dataset_root, "images")
            lbl_dir = os.path.join(dataset_root, "labels")
            if not (os.path.isdir(img_dir) and os.path.isdir(lbl_dir)):
                raise RuntimeError(
                    f"No split-like or flat-pool structure found under {dataset_root}"
                )
            for fname in os.listdir(img_dir):
                stem, ext = os.path.splitext(fname)
                label = os.path.join(lbl_dir, stem + ".txt")
    ...
    if do_rebalance:
        ...
        dataset_root_for_split = pool_dir  # this can be a flat pool or split root
        rebalance_dataset(
